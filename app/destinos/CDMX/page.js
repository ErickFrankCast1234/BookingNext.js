...
export default function CiudadDeMexicoPage() {
  ...
  const fetchHoteles = async ({ servicios = [], tipos = [] } = {}, ordenSeleccionado = "") => {
    try {
      ...
      if (ordenSeleccionado) {
        url.searchParams.append("orden", ordenSeleccionado);
      }
  ...
  return (
    <>
      {/* CABECERA OMITIDA PARA BREVEDAD */}
      {/* Cabecera responsiva */}
      ...
      <main className="min-h-screen bg-gray-100 pt-18">
        ...
        <div className="flex justify-around sm:hidden my-4 px-2">
          <button
            className="flex items-center gap-1 px-3 py-2 bg-white text-blue-600 rounded-md font-medium shadow"
            onClick={() => document.getElementById("ordenar-modal")?.showModal()}
          >
          ...
          <button
            className="flex items-center gap-1 px-3 py-2 bg-white text-blue-600 rounded-md font-medium shadow"
            onClick={() => document.getElementById("filtros-modal")?.showModal()}
          >
          ...
          <button
            className="flex items-center gap-1 px-3 py-2 bg-white text-blue-600 rounded-md font-medium shadow"
            onClick={() => document.getElementById("mapa-modal")?.showModal()}
          >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6 mt-4">
            {/* MAPA Y FILTROS LATERALES EN ESCRITORIO */}
            <div className="hidden lg:flex flex-col gap-4 h-fit sticky top-24">
              <iframe
                ...
                loading="lazy"
              ></iframe>
              <HotelFilterBar destino="Ciudad de México" onFilterChange={handleFilterChange} />
            </div>
